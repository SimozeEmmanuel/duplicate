        ...
        if form.is_valid():
            try:
                ...
                messages.add_message(request, messages.WARNING,
                                     "The auction you seek is unavailable. It has not been created or the auction is closed.")
                return render(request, "auctions/create_listing.html",
                              {"form": form})
            # Successfully created a Listing
            messages.add_message(request, messages.SUCCESS,
                                 "Your auction was successfully processed and created!")
...
def _get_listing_dict(list_args):
    ...
    try:
        ...
        raise Http404("This auction does not exist.")
...
def place_bid(request):
    ...
    if form.is_valid():
        try:
            ...
            messages.add_message(request, messages.WARNING,
                                 "Your bid did not proccess. Please try again")
            return HttpResponseRedirect(
                reverse("listing", args=(bid_listing.id,)))
        # Saving the bid was successful
        messages.add_message(request, messages.SUCCESS,
                             "Your bid was successfuly proccessed and accepted. Congratulations!")
...
def add_comment(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS,
                "The comment was successfully posted. Others visiting this auction will now be able to see your comment."
            )
            return HttpResponseRedirect(reverse("listing", args=(cmt_listing.id, )))
